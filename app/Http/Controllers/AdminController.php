...
class AdminController extends Controller
{
    ...
    public function storeAbout(Request $request)
    {
        ...
        if ($request->hasFile('rightImg')) {
            ...
            $about->rightImg = 'http://ds.dstudio.com.bd/public/Photo/about/' . $rightImgName;
        }
        ...
        if ($request->hasFile('leftImg')) {
            ...
            $about->leftImg = 'http://ds.dstudio.com.bd/public/Photo/about/' . $leftImgName;
        }
    ...
    // Create a new instance of RuralService (always create a new record)
    $ruralService = new UrbanService();
    ...
    foreach (['imgOne', 'imgTwo', 'imgThree', 'imgFour'] as $imgField) {
        if ($request->hasFile($imgField)) {
            // If a file is uploaded, handle the image
            ...
            $imagePaths[$imgField] = 'http://ds.dstudio.com.bd/public/Photo/service/urban/' . $imgName;
        } else {
            // If no image is uploaded, set it to null or keep the old one (optional)
            $imagePaths[$imgField] = null;
        }
    }

    // Assign the details and image paths to the new RuralService instance
    ...
    // Save the new RuralService record to the database
    $ruralService->save();

    // Redirect with a success message
    return redirect()->back()->with('success', 'New Urban Service data saved successfully!');
    }
    ...
    public function storeUpIdea(Request $request)
    {
        ...
        foreach (['imgOne', 'imgTwo', 'imgThree', 'imgFour'] as $imgField) {
            if ($request->hasFile($imgField)) {
                ...
                $imagePaths[$imgField] = 'http://ds.dstudio.com.bd/public/Photo/idea/' . $imgName;
            } else {
 ...
 public function storeRuralService(Request $request)
        ...
        try {
            ...
            foreach (['imgOne', 'imgTwo', 'imgThree', 'imgFour'] as $imgField) {
                if ($request->hasFile($imgField)) {
                    ...
                    $imagePaths[$imgField] = 'http://ds.dstudio.com.bd/public/Photo/service/rural/' . $imgName;
                } else {
                    $imagePaths[$imgField] = null;
                }
            }
    ...
    // Create a new instance of RuralService (always create a new record)
    $ruralService = new RemoteService();
    ...
    foreach (['imgOne', 'imgTwo', 'imgThree', 'imgFour'] as $imgField) {
        if ($request->hasFile($imgField)) {
            ...
            $imgFile = $request->file($imgField);
            $imgName = time() . '_' . $imgFile->getClientOriginalName();
            $imgFile->move(public_path('Photo/service/remote/'), $imgName);
            $imagePaths[$imgField] = 'http://ds.dstudio.com.bd/public/Photo/service/remote/' . $imgName;
        } else {
            // If no image is uploaded, set it to null or keep the old one (optional)
            $imagePaths[$imgField] = null;
        }
    ...
    // Redirect with a success message
    return redirect()->back()->with('success', 'New Remote Service data saved successfully!');
    }
